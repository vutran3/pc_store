...
const AdminChatTab = () => {
    ...
    useEffect(() => {
        ...
        messageApi.getMessages(selected.id)
            .then((msgs) => {
    ...
    // ✅ Xóa socket listener riêng vì SocketClient đã handle
    ...
    return (
        <div style={{ display: 'flex', height: '70vh', background: '#fff', borderRadius: 12, boxShadow: '0 2px 8px #0001' }}>
            ...
            <div style={{ width: 320, borderRight: '1px solid #eee', overflowY: 'auto' }}>
                ...
                {conversations.length === 0 && (
                    <div style={{ padding: 24, color: '#888' }}>Không có hội thoại nào</div>
                )}
            ...
            <div style={{ flex: 1, display: 'flex', flexDirection: 'column', height: '100%' }}>
                {!selected ? (
                    ...
                    <>
                                ...
                                messagesFromRedux.map((msg) => (
                                    ...
                                    >
                                        ...
                                        <div style={{ fontSize: 10, color: '#aaa', marginTop: 2 }}>
                                            {msg.sender?.userName} • {msg.createdDate ? new Date(msg.createdDate).toLocaleString('vi-VN') : ''}
                                        </div>
                                    </div>
                                ))
                            )}
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', padding: 16, borderTop: '1px solid #eee', background: '#fff' }}>
                            <input
                                type="file"
                                accept="image/*"
                                style={{ display: 'none' }}
                            ...
                            />
                            <label htmlFor="chat-image-upload" style={{ cursor: 'pointer', marginRight: 8 }} title="Gửi ảnh">
                            ...
                            <input
                                ...
                                style={{ flex: 1, border: '1px solid #eee', borderRadius: 16, padding: '8px 16px', marginRight: 8 }}
                                onKeyDown={e => { if (e.key === 'Enter') handleSend(); }}
                            />
                            <button
                                onClick={handleSend}
                                style={{ background: '#6c47ff', color: '#fff', border: 'none', borderRadius: 16, padding: '8px 20px', fontWeight: 600, cursor: 'pointer' }}
