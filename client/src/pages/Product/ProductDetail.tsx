	...
	const handleAddToCart = async () => {
		...
		if (!effectiveCustomerId) {
			...
			if (token) {
				...
				if (effectiveCustomerId) {
					try { localStorage.setItem('userId', effectiveCustomerId); } catch { }
				}
			}
		}
		...
		if (!effectiveCustomerId) {
			const possible = localStorage.getItem('user') || localStorage.getItem('authUser') || localStorage.getItem('userInfo');
			if (possible) {
				try {
		...
		try {
			...
			try { await dispatch(fetchCart(effectiveCustomerId)); } catch (e) { /* ignore */ }
